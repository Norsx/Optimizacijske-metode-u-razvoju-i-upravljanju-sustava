...
Prvo provjeravamo je li ograničenje uopće aktivno. Bezuvjetni minimum je
$x_{\text{unc}} = -H^{-1}c = (-0{,}897598,\; 0{,}682680)^{\mathsf T}$, za koji je
$x_1 + x_2 = -0{,}215 < 10$. Ograničenje \eqref{eq:z1g1} je dakle prekršeno i mora
biti aktivno u optimumu.

Pretpostavimo da je aktivno samo prvo ograničenje, tj. $\lambda_2 = 0$. Tada
\eqref{eq:kkt1} i $A_1x = b_1$ daju linearan sustav
\begin{equation}
  \begin{bmatrix} H & A_1^{\mathsf T} \\ A_1 & 0 \end{bmatrix}
  \begin{bmatrix} x \\ \lambda_1 \end{bmatrix}
  = \begin{bmatrix} -c \\ b_1 \end{bmatrix},
\end{equation}
čije je rješenje
\begin{equation}
...
Pretpostavku treba potvrditi. Dualna dopustivost vrijedi jer je
$\lambda_1^\star = 12{,}242424 > 0$. Drugo ograničenje je neaktivno jer je
$x_1^\star - x_2^\star - 3 = -1{,}484848 < 0$, što je konzistentno s
$\lambda_2^\star = 0$ u \eqref{eq:kkt4}. Rezidual stacionarnosti iznosi
...
\begin{equation}
  ...
  \nabla g_1(x^\star) = \begin{bmatrix} -1 \\ -1 \end{bmatrix}.
\end{equation}
...
\begin{figure}[htbp]
  ...
  ograničenje (puna crta), neaktivno ograničenje (crtkano) i gradijenti u
  točki $x^\star$. Crvena krivulja je nivo krivulja $f(x) = p^\star$.}
  \label{fig:z1}
\end{figure}
