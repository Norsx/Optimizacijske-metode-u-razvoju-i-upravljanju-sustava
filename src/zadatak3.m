...
%
% Trosak je proporcionalan kolicini premjestenog materijala, dakle
% sum_i |y_i - h(x_i)| * dx.  Uz linearna ogranicenja na nagib i promjenu
% nagiba to je LINEARNI PROGRAM (nakon epigrafske reformulacije
% |y_i - h_i| <= t_i, minimiziraj sum t_i).
%
...
for k = 1:4
    ...
    title(sprintf('slucaj %s, N = %d  (b_1 = %.1f, b_2 = %.1f),  trosak = %.3f', ...
...
% LP:  varijable z = [y_0..y_N , t_0..t_N],  min sum(t)*dx
    dx = L / N;
    n = N + 1;                 % broj cvorova
    nz = 2*n;                  % y i t
    x = linspace(0, L, n)';
